...
export function useMakeTypeDraft(
	...
	const draft = ref<Draft>(
		isNewMode.value
			? emptyDraftForNewMode('Make/Instances')  // settings.defaultInstancesRoot — pulled from store in MakeType.vue
			: (committedType.value !== null ? toDraft(committedType.value) : emptyDraftForNewMode('Make/Instances')),
	);
	...
	function resetDraft(): void {
		if (isNewMode.value) {
			draft.value = emptyDraftForNewMode('Make/Instances');
...
Note: the `defaultInstancesRoot` is hardcoded to `'Make/Instances'` in the composable. `MakeType.vue` (Task 3.17) will pass the real value from `getMakeSettings()?.defaultInstancesRoot` via a modified composable signature, OR the composable can read settings itself. For now keep the hardcoded value — Task 3.17 adjusts the signature.
...
const errorText = computed(() => props.errors.map((e) => JSON.stringify(e)).join('; '));
...
Props: `v-model:draft: Draft`, `mode: 'new' | 'edit'`, `isDirty: boolean`, `isSaving: boolean`, `serviceError: string | null`, `hasExistingInstances: boolean`, `originalFolder?: string`, `fieldErrors: Map<string, FieldError[]>`.
...
async function surfaceError(error: MakeError): Promise<void> {
	if (error.kind === 'invalid-schema') {
		...
		// Generic invalid-schema: populate fieldErrors + focus first invalid.
		// (Per-issue routing is delegated to MakeTypeFieldsEditor via fieldErrors.)
		// Focus first invalid input:
		nextTick(() => {
			document.querySelector<HTMLInputElement>('[aria-invalid="true"]')?.focus();
		});
	}
	// Other error kinds (vault-error, duplicate-name, type-not-found) already surface
	// via store.saveError / notifications; MakeTypeFieldsEditor's serviceError prop
	// renders the banner.
}
