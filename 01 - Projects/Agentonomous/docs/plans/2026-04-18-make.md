...
Add the `import type` line at the **top** of `types.ts` (with any other imports) and the two new type declarations at the **bottom** of the file (after the existing declarations). Keep imports-at-top — appending imports mid-file works at runtime but violates the project's `import/first` lint rule.

Add to the top of `types.ts`:
...
## Chunk 2: Slice 2 — Read pages ✅ Shipped

**Tag**: `make-slice-2` (merged to master 2026-04-18).
**Spec**: `docs/specs/2026-04-18-make-chunk-2-design.md`.
**Plan**: `docs/plans/2026-04-18-make-chunk-2.md`.

Delivered: `/make`, `/make/types`, `/make/types/:typeId` routes; `MakeHome`, `MakeTypes`, consolidated `MakeType` with Fields/Instances tabs (replacing the parent spec's separate `MakeTypeConfig` + `MakeTypeIndex`); `make-view.ts` Obsidian wrapper; Pinia store with listTypes/listInstances caching; real `MakeService.listInstances` implementation. Refresh is manual. Corrupt files remain log-only.

## Chunk 3: Slice 3 — Type authoring ✅ Shipped

**Tag**: `make-slice-3` at `ba9ae360` (merged to master 2026-04-18).
**Spec**: `docs/specs/2026-04-18-make-chunk-3-design.md`.
**Plan**: `docs/plans/2026-04-18-make-chunk-3.md`.

Delivered: `createType`/`updateType`/`deleteType`/`regenerateBaseFile`/`toggleFavorite` service methods (all emitting typed `make:*` events via reshaped `make-events.ts`); editable Fields tab with Save/Cancel footer and full validation flow; `DeleteTypeDialog` with instance-count info + base-file checkbox; base-file missing/stale banner with user-edit-protected regenerate; optimistic clickable favorite star; `/make/types/new` route; full `t()` wiring on all Make pages (~65 new i18n keys); 4 ARIA-compliant dialogs (ConfirmDialog shared primitive); complete keyboard-accessible tab pattern. Tests: 533 → 740 (+207). See spec §12 for the **Chunk 3.5 hardening outbox** (11 deferred items — optimistic concurrency, field-rename drift, etc.) recommended before Chunk 4.

## Chunk 3.5: Hardening (placeholder — before Chunk 4)

Operational hardening deferred from Chunk 3 review. Scoped in `docs/specs/2026-04-18-make-chunk-3-design.md` §12. Not yet spec'd as a standalone plan — recommended sequence: spec → plan → execute.

Scope summary: optimistic concurrency via `stat.mtime` on updateType/deleteType; save-time notices for field-kind changes and title-field renames; persistent post-save orphan banner after `instancesFolder` rename; relative `baseFile.path` storage; granular per-field rename count; `PluginContext`-based refactor of `getMakeService`/`getMakeSettings`/`subscribeMakeEvents`; `VaultPort` atomic-write contract; canonical-form base-file divergence check; external-update-during-dirty-draft recovery; external-delete-during-edit recovery; favorite multi-session sync recovery.

## Chunk 4: Slice 4 — Instance authoring (placeholder)

Chunk 4 will implement instance CRUD service methods, the submission form on `MakeTypeInstances`, cascade delete (re-enabling the checkbox in `DeleteTypeDialog` deferred from Chunk 3), and "Open in Obsidian". Also surfaces corrupt type files in the UI (deferred from Chunks 2 and 3).

**Dependencies**: recommended to land Chunk 3.5 first so write-path concurrency is handled before instances compound the risk.
...
## Appendix A: Chunk 1 — Deviations from plan text

Documented as a reference for authors working on Chunks 2–5; Chunk 1 verbatim code in this file should NOT be copied without also reading this section.

**Scaffolding**
- Task 1.1 `.gitkeep` files were not created. Empty folders can't be tracked by git; each later task created real files in their target directories. `src/ui/pages/make/` and `stories/pages/make/` do not yet exist and will be created in Chunk 2.

**Domain — text spec (commit `85cb02c3`)**
- `text.ts`: `validateValue` treats `undefined`/`null` as `required-missing` when required (plan had `invalid-text` for this case). The fix aligns with number/date/datetime which already had the `required-missing` branch.

**Domain — lint-driven refactors (commit `d5953baf`)**
- `make-service.ts`: raw `try { JSON.parse } catch` replaced with `trySync` from `src/domain/shared/try-async.ts`. The `no-restricted-syntax` ESLint rule bans try/catch outside `src/infrastructure/**`.
- `make-service.ts`: `notImpl`, `toggleFavorite`, `getKpis` stubs changed from `async () => ...` to `() => Promise.resolve(...)` — `@typescript-eslint/require-await` rejects async functions that never await.
- `type-schema-codec.ts` line 53: `String(titleField)` replaced with `JSON.stringify(titleField)` to avoid `no-base-to-string`. See below — further changed during review to return `invalid-json` instead.

**UI — Vue component refinements (commit `5843d5a6`)**
- `ListInput.vue`: `@keydown.enter.prevent="add" @keydown.comma.prevent="add"` was rewritten as a single `@keydown="onDraftKeydown"` named handler. Vue has no `comma` keydown modifier; only named keys like `enter` are supported.
- `CheckboxInput.vue`: dropped unused `const props = defineProps<{…}>()` assignment — template-only field access does not need the identifier.
- All inputs: `<input ... />` → `<input ...>` (HTML void-element convention) and multi-line attribute formatting — applied by `eslint --fix`.

**Post-review fixes (commits `106773fc`, `5fa850c7`)**
- `sanitize-filename.ts`: added Windows reserved-name rejection (`CON`, `PRN`, `AUX`, `NUL`, `COM1–9`, `LPT1–9`, case-insensitive) — returns empty stem so the default-name fallback fires.
- `type-schema-codec.ts` line 53: non-string `titleFieldName` now returns `err({ kind: 'invalid-json', ... })` instead of `title-field-missing`. The shape is wrong, not the target.
- `field-kind-spec.ts`: JSDoc on `validateValue` now documents the asymmetric empty-value contract — `text`/`list` return `ok(empty sentinel)` for optional-empty, `number`/`date`/`datetime` return `err(invalid-*)` because `FieldValue` has no null variant. Slice 4 (instance form submission) owns the pre-strip responsibility.
- `make-module.ts`: dropped the dead `currentSettings` fallback; service closure reads `state.settings` directly and throws if called post-destroy. `init` now awaits `destroy()` instead of fire-and-forget.
- `date.ts`: `parseLocalDate` is now exported so `DateInput.vue` can reuse it — catches calendar-invalid dates (e.g., `2023-02-30`) set programmatically.
- `CheckboxInput.vue`: wrapped `[checkbox][label]` in an inner row container; description and error now stack below the row instead of alongside.
- `ListInput.vue`: duplicate entries surface a transient inline hint instead of silently clearing the draft.
- `{Number,Date,Datetime,Checkbox,List}Input.vue`: added `data-testid="input-help"` to the description span (TextInput already had it).
- `CheckboxInput.stories.ts`: added `WithError` and `WithDescription` stories for parity with the other five input components.
