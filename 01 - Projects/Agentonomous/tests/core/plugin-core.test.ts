...
import { fakeSettings, fakeCommands, fakeViews, fakeNotifications, fakeDialogs, fakeLogger, fakeTranslation, fakePlatform, fakeVault, fakeStorage, fakeScheduler, fakeAgents, fakeTasks } from '../__fakes__/fake-ports.js';
...
describe('PluginCore with modules', () => {
	it('init() calls module.init in dependency order', async () => {
		...
		const core = new PluginCore(
			{ settings: fakeSettings(), commands: fakeCommands(), views: fakeViews(), logger: new Logger(bus, 'error'), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[b, a],
		);
		await core.init();
	...
	it('collects and registers commands from all modules', async () => {
		...
		const core = new PluginCore(
			{ settings: fakeSettings(), commands, views: fakeViews(), logger: new Logger(bus, 'error'), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[m],
		);
		await core.init();
	...
	it('passes validated settings to module.init', async () => {
		...
		const core = new PluginCore(
			{ settings, commands: fakeCommands(), views: fakeViews(), logger: new Logger(bus, 'error'), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[m],
		);
		await core.init();
...
describe('PluginCore graceful degradation', () => {
	it('continues initializing other modules when one throws', async () => {
		...
		const core = new PluginCore(
			{ settings: fakeSettings(), commands: fakeCommands(), views: fakeViews(), logger: fakeLogger(), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[broken, healthy],
		);
		await core.init();
		expect(core.ready).toBe(true);
	...
	it('does not register commands for failed modules', async () => {
		...
		const core = new PluginCore(
			{ settings: fakeSettings(), commands, views: fakeViews(), logger: fakeLogger(), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[broken],
		);
		await core.init();
...
describe('PluginCore listener leak detection', () => {
	it('warns when a module leaks listeners during destroy', async () => {
		...
		const core = new PluginCore(
			{ settings: fakeSettings(), commands: fakeCommands(), views: fakeViews(), logger, notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[leaky],
		);
		await core.init();
		await core.destroy();
...
describe('PluginCore core settings resolution', () => {
	it('falls back to defaults when core settings are invalid in blob', async () => {
		...
		const m = defineModule({ id: 'a', name: 'A', async init() {}, destroy() {} });
		const core = new PluginCore(
			{ settings: settingsPort, commands: fakeCommands(), views: fakeViews(), logger: fakeLogger(), notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[m],
		);
		await core.init();
...
describe('PluginCore onSettingsChange dispatch', () => {
	...
	it('logs error and continues when onSettingsChange throws', async () => {
		...
		const core = new PluginCore(
			{ settings, commands: fakeCommands(), views: fakeViews(), logger, notifications: fakeNotifications(), dialogs: fakeDialogs(), eventBus: bus, t: fakeTranslation(), platform: fakePlatform(), vault: fakeVault(), storage: fakeStorage(), scheduler: fakeScheduler(), agents: fakeAgents(), tasks: fakeTasks() },
			[m],
		);
		await core.init();
