...
Markdown+frontmatter files in `src/content/onboarding/tours/`:

```
src/content/onboarding/
├── welcome.md
├── tours.json
├── tours/
│   └── project-manager/
...
Persists `TourProgress` to `.flowti/var/onboarding-progress.json`. PM can quit and resume where they left off.
...
| 2 | `narrate` | Tour selection | Shows available tours, PM picks "Project Manager" |
...
1. Tour engine sets an **onboarding context** flag: `{ tourId, stepId, returnTo }`
2. Navigation goes to the real sitemap page (e.g. `iteration-planning`)
3. The page renders normally, but onboarding context triggers:
   - Alice's hints appear (before-render hook)
   - Subtle banner: "You're in the PM tour — press `b` when done to continue"
4. When PM presses back, onboarding context intercepts and returns to tour flow at the next step
