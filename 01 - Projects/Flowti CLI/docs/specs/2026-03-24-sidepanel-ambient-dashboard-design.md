...
- 3px ring colored by current intent (STATE_COLORS: working=#22c55e, idle=#3b82f6, on-break=#a855f7, talking=#06b6d4, seeking=#6b7280, waiting=#f59e0b)
- When any need drops below 25%: a small pulsing dot (4px, red) appears at bottom-right of the portrait
...
1. Rail collapses to 56px (200ms)
2. Stores previous expanded/collapsed state
3. When the slide panel closes, rail restores to previous state

This ensures the slide panel's 60% width doesn't fight with an expanded rail.
...
- Health color: determined by the lowest need value
  - All needs >= 60%: `var(--accent-green)` at 0.35 fill, 0.8 stroke
  - Any need 25-59%: `var(--accent-gold)` at 0.35 fill, 0.8 stroke (warning)
  - Any need < 25%: `var(--accent-red)` at 0.4 fill, 0.9 stroke (critical)
- No labels, no axis lines, no numbers — pure shape recognition

**Helper function:** `renderNeedsRadar(needs: AgentNeeds, size: number): SVGTemplateResult` — placed in `game-ui-constants.ts` since it's a shared visual helper.
...
- Panel top-border: 2px accent color at the very top of the panel, color varies by panel type:
  - agent-detail: `var(--accent-gold)`
  - bob: `var(--accent-blue)`
  - roster: `var(--accent-green)`
  - merchant: `var(--accent-amber)`
...
| File | Change |
|------|--------|
| `src/game/ui/sidebar.ts` | Rewrite: collapsible rail, mini cards, radar, SVG icons, auto-collapse logic |
...
| `src/game/ui/game-ui-constants.ts` | Add `renderNeedsRadar()` helper, radar geometry constants |
| `tests/game/ui/sidebar.test.ts` | New: test collapsed/expanded rendering, auto-collapse, click handlers |
| `tests/game/ui/game-ui-constants.test.ts` | Add: radar helper tests (healthy/warning/critical shapes) |
