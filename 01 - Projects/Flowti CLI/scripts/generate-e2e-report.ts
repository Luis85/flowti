...
const JOURNEYS_DIR: string = path.join(TEST_VAULT, "docs", "journeys");

// Development vault — separated by artifact type
const DEV_RUNS_DIR: string = path.join(PLUGIN_ROOT, "docs", "reports", "e2e", "runs");
const DEV_TRACES_DIR: string = path.join(PLUGIN_ROOT, "docs", "reports", "e2e", "traces");
const DEV_JOURNEYS_DIR: string = path.join(PLUGIN_ROOT, "docs", "journeys");
...
function readVitestResults(): VitestResults | null {
	...
	for (const file of (raw.testResults as Record<string, unknown>[] ?? [])) {
		const suiteName = path.basename(file.name as string, ".test.ts");
		const cases: VitestCase[] = [];
		...
		for (const test of (file.assertionResults as Record<string, unknown>[] ?? [])) {
			const status = (test.status as string) ?? "unknown";
		...
		if (suiteHookFailed) {
			hookError = (file.message as string)
				|| ((file.assertionResults as Record<string, unknown>[] | undefined)?.find((t) => (t.failureMessages as string[] | undefined)?.length)?.failureMessages as string[] | undefined)?.[0]
...
function reconcileResults(vitest: VitestResults | null, journeys: JourneyEntry[]): VitestResults | null {
	...
	const journeyStepMap = new Map<string, Array<{ itBlock: string; status: string }>>();
	...
	for (const suite of vitest.suites) {
		...
		let matchedJourney: { name: string; steps: Array<{ itBlock: string; status: string }> } | null = null;
...
function generateJourneyReport(data: Record<string, unknown>, date: string): JourneyReportResult {
	...
	doc.mergeFrontmatter({
		...
		duration: formatDuration((data.durationMs as number) ?? 0),
	...
	doc.addBlank()
		...
		.addBlank();
	doc.callout(statusCallout(journeyStatus), `${statusLabel(journeyStatus)} — ${stepsSummary} | ${formatDuration((data.durationMs as number) ?? 0)}`, [
	...
	function renderStep(stepResult: StepResult): void {
...
function generateJourneyCanvas(data: Record<string, unknown>, screenshotBasePath: string, trace: TraceData | null, configFilePath: string | null): CanvasResult {
	const nId = (key: string): string => `${CANVAS_PREFIX}n-${key}`;
	const gId = (key: string): string => `${CANVAS_PREFIX}g-${key}`;
	const eId = (from: string, to: string): string => `${CANVAS_PREFIX}e-${from}-${to}`;
	...
	const canvasStatusLabel = (status: string): string => {
		if (status === "pass") return "PASS";
		if (status === "fail") return "FAIL";
		return "SKIP";
	};

	const canvasVars = (data.variables as Record<string, string>) ?? {};

	const journeySlug = (data.journey as string) ?? "unknown";
	const journeyTitle = journeySlug
		.replace(/-/g, " ")
		.replace(/\b\w/g, (c: string) => c.toUpperCase());
	const steps = (data.steps as StepResult[]) ?? [];
	const passedSteps = (data.passed as number) ?? 0;
	const failedSteps = (data.failed as number) ?? 0;
	const skippedSteps = (data.skipped as number) ?? 0;
	const totalSteps = (data.totalSteps as number) ?? 0;
	...
	nodes.push({
		id: nId("start"),
		type: "text",
		text: `# Start\n**${journeyTitle}**\n${(data.date as string)?.substring(0, 10) ?? ""}`,
	...
	for (let i = 0; i < steps.length; i++) {
		...
		const describeStr = s.describeBlock ?? (data.journey as string) ?? "";
		...
		if (actions.length > 0) {
			...
			for (let ai = 0; ai < actions.length; ai++) {
				...
				let actionBackground: string | null = null;
				if (isScreenshot) {
					const label = action.label ?? String(++screenshotCounter);
					actionBackground = `${screenshotBasePath}/${s.id}--${label}.png`;
				}

				const actionGroupNode: CanvasNode = {
					id: gId(actionId),
					type: "group",
					label: formatActionText(action, data.variables as Record<string, string> | undefined),
					x: actionCenterX,
					y: actionY,
	...
	const eventsLines: string[] = ["## Events Summary"];
	eventsLines.push(`**Steps**: ${passedSteps} passed, ${failedSteps} failed`);
	eventsLines.push(`**Duration**: ${formatDuration((data.durationMs as number) ?? 0)}`);
	eventsLines.push("");
	...
	const stepsSummary = skippedSteps > 0
		? `${passedSteps}/${totalSteps} steps (${skippedSteps} skipped)`
		: `${passedSteps}/${totalSteps} steps`;
	const endText = `# ${endLabel}\n${stepsSummary}\n${formatDuration((data.durationMs as number) ?? 0)}`;
...
function buildPerfEventStats(perfEvents: PerfTraceEvent[], doc: InstanceType<typeof Document>): void {
	...
	let startupTotal: StartupTotal | null = null;
	const storageOps: StorageOp[] = [];
	const queries: QueryOp[] = [];
	const dispatches: DispatchOp[] = [];
	const alerts: AlertOp[] = [];

	for (const e of perfEvents) {
		let p: Record<string, unknown>;
		try {
			p = typeof e.payload === "string" ? JSON.parse(e.payload) as Record<string, unknown> : e.payload as Record<string, unknown>;
		} catch {
			continue;
		}
		...
		switch (e.type) {
			case "perf.startup.service":
				if (p.service && p.durationMs !== undefined) {
					startupServices.push({ service: p.service as string, durationMs: p.durationMs as number });
				}
				break;
			case "perf.startup.total":
				if (p.durationMs !== undefined) {
					startupTotal = { durationMs: p.durationMs as number, serviceCount: (p.serviceCount as number) ?? 0 };
				}
				break;
			...
			case "perf.storage.saved":
				if (p.key && p.durationMs !== undefined) {
					storageOps.push({
						...
						sizeBytes: (p.sizeBytes as number) ?? 0,
					});
				}
				break;
			case "perf.query.executed":
				if (p.queryId && p.durationMs !== undefined) {
					queries.push({
						queryId: p.queryId as string,
						durationMs: p.durationMs as number,
						sourceRows: (p.sourceRows as number) ?? 0,
						resultRows: (p.resultRows as number) ?? 0,
					});
				}
				break;
			case "perf.event.dispatched":
				if (p.eventType && p.durationMs !== undefined) {
					dispatches.push({
						...
						handlerCount: (p.handlerCount as number) ?? 0,
						durationMs: p.durationMs as number,
					});
				}
				break;
	...
	if (storageOps.length > 0) {
		...
		const rows: string[][] = display.map((o) => {
...
function generateReport(): void {
	...
	for (const { dir, data } of journeys) {
		...
		const devContent = content.replace(
			/!\[\[([^\]]+\.png)\]\]/g,
			(_: string, file: string) => `![](screenshots/${file})`,
		);
	...
	const hasJourneyWarnings = journeys.some(({ data }) =>
		((data.steps as StepResult[]) ?? []).some((r) => r.warnings && r.warnings.length > 0));
	...
	const perJourneyStats = new Map<string, ActionStatsReturn>();
	for (const { data } of journeys) {
		...
		perJourneyStats.set(data.journey as string, stats);
	}
	...
	const vitestFailures: Array<{ suite: string; testCase: VitestCase; hookError: string | null }> = [];
	if (vitest) {
		for (const suite of vitest.suites) {
			for (const c of suite.cases) {
	...
	if (journeyReportNames.length > 0) {
		...
		for (const { title, data } of journeyReportNames) {
			...
			const jDevSteps = (data.dev as number) ?? 0;
			const jStatus = resolveStatus((data.passed as number) ?? 0, (data.failed as number) ?? 0, (data.totalSteps as number) ?? 0, jSkipped, false, jDevStopped);
			const stats = perJourneyStats.get(data.journey as string);
			const jTitleSuffix = jStatus === "partial-pass" ? " (Partial)"
				: jStatus === "dev-stopped" ? " (Dev)" : "";
			const jStepsSummary = jDevStopped
				? `${(data.passed as number) ?? 0}/${(data.totalSteps as number) ?? 0} steps (${jDevSteps} dev, ${jSkipped} skipped)`
				: jSkipped > 0
					? `${(data.passed as number) ?? 0}/${(data.totalSteps as number) ?? 0} steps (${jSkipped} skipped)`
					: `${(data.passed as number) ?? 0}/${(data.totalSteps as number) ?? 0} steps`;
			...
			doc.callout(statusCallout(jStatus), `${statusLabel(jStatus)} — ${jStepsSummary} | ${formatDuration((data.durationMs as number) ?? 0)}`, jCalloutLines);
			doc.addBlank();
	...
	for (const { dir, data } of journeys) {
		try {
			const resultsFile = path.join(dir, `${(data.journey as string)}-results.json`);
