...
export function wrapHtml(title: string, body: string, deps: Pick<ReportDeps, "clock">, meta?: Record<string, string | number>): string {
...
Generated by Flowti CLI &middot; ${deps.clock.iso().split("T")[0]}
</footer>
</body>
</html>`;
}
 ...
 */
export function exportReportToHtml(mdPath: string, outputDir: string, deps: ReportDeps): HtmlExportResult | null {
	if (!deps.disk.existsSync(mdPath)) return null;

	const raw = deps.disk.readFileSync(mdPath, "utf-8");
	const split = splitFrontmatter(raw);
	const frontmatter = split?.frontmatter ?? {};
	const body = split?.body ?? raw;

	const title = frontmatter.title ?? deps.paths.basename(mdPath, ".md");
	...
	const html = wrapHtml(title, htmlBody, deps, Object.keys(meta).length > 0 ? meta : undefined);

	const slug = deps.paths.basename(mdPath, ".md");
	const outputPath = deps.paths.join(outputDir, `${slug}.html`);
	deps.disk.mkdirSync(outputDir, { recursive: true });
	deps.disk.writeFileSync(outputPath, html, "utf-8");
