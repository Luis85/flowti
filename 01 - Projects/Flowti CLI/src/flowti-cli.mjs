...
// ══════════════════════════════════════════════════════════════════════
// Constants — Kernel-Space Path Resolution
// ══════════════════════════════════════════════════════════════════════

const CLI_DIR = import.meta.dirname;
const CLI_PROJECT = path.resolve(CLI_DIR, "..");
const CLI_CONFIG = JSON.parse(fs.readFileSync(path.join(CLI_PROJECT, "configs", "flowti-cli.config.json"), "utf-8"));
const VAULT_ROOT = path.resolve(CLI_PROJECT, "..", "..");
const PLUGIN_ROOT = path.resolve(VAULT_ROOT, CLI_CONFIG.subsystems?.plugin?.root ?? "Development/flowti");

const ROOT = PLUGIN_ROOT;
const CONFIG_PATH = path.join(ROOT, CLI_CONFIG.subsystems?.plugin?.config ?? "flowti.config.json");
const MANIFEST_PATH = path.join(ROOT, CLI_CONFIG.subsystems?.plugin?.manifest ?? "manifest.json");
const PKG_PATH = path.join(ROOT, CLI_CONFIG.subsystems?.plugin?.package ?? "package.json");
...
function showPostBuildGuidance() {
	const mainJs = path.join(VAULT_ROOT, ".obsidian", "plugins", "flowti-ibde", "main.js");
