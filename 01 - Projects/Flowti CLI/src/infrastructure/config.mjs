...
const CLI_PROJECT = path.resolve(CLI_DIR, "..", "..");      // 01 - Projects/Flowti CLI/
...
export const PLUGIN_ROOT = path.resolve(VAULT_ROOT, CLI_CONFIG.subsystems?.plugin?.root ?? "Development/flowti");
export const ROOT = PLUGIN_ROOT;

export const CONFIG_PATH = path.join(ROOT, CLI_CONFIG.subsystems?.plugin?.config ?? "flowti.config.json");
