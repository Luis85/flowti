...
class NodeShell implements IShell {
	run(cmd: string, opts: { cwd?: string; label?: string; env?: Record<string, string>; nonInteractive?: boolean } = {}): number {
		...
		try {
			const stdio = opts.nonInteractive ? ["ignore" as const, "inherit" as const, "inherit" as const] : "inherit" as const;
			execSync(cmd, { cwd, stdio, env: opts.env ? { ...process.env, ...opts.env } : undefined });
