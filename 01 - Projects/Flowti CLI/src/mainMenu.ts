...
function buildToolItems(projectPath: string, config: ProjectConfig): MenuEntry[] {
	...
	return FLOWTI_TOOLS.map((def) => {
		...
		if (cmd) {
			return {
				key: def.key,
				label: def.label,
				action: () => { runIn(cmd, projectPath, def.label); return "main" as const; },
			};
		}
		return {
			key: def.key,
...
function buildScriptItems(projectPath: string, scripts: Record<string, string>): MenuEntry[] {
	...
	return names.map((name) => ({
		key: name,
		label: `npm run ${name}`,
		action: () => { runIn(`npm run ${name}`, projectPath, name); return "main" as const; },
	}));
}
...
export function buildProjectDetailMenu(): MenuEntry[] {
	...
	// Flowti tools (top-level, disabled if unmapped)
	...
	if (scriptItems.length > 0) {
		items.push({
			key: "r", label: "Run npm script...", action: async () => {
	...
	items.push(
		...
		{ key: "s", label: "Project Status Report", action: async () => { await generateProjectStatusReport(); return "main" as const; } },
	);
	...
	// Navigation
	items.push(
		{ key: "i", label: "Info", action: () => { showInfo(); return "main" as const; } },
