...
export async function runBriefInteractive(
	...
	const { renderAgentSpawned, renderAgentOutput } = await import("../displays/agent-run-display.js");
	...
	handle.subscribe((event) => renderAgentOutput(event, deps.log));
	await handle.process.waitForOutput(/.*/, 300000);
...
async function spawnAndStream(
	...
	const { buildRunSpec } = await import("../../domain/agents/agent-runner.js");
	const { checkClaudeInstalled, launchAgent } = await import("../../infrastructure/agent-process.js");
	const { createSession, updateSessionStatus, appendOutput } = await import("../../domain/agents/agent-session.js");
	const { renderAgentSpawned, renderAgentOutput } = await import("../displays/agent-run-display.js");
	if (!checkClaudeInstalled(deps)) {
		deps.log("\n  Claude CLI is not installed or not in PATH.\n");
		return;
	}
	// Agent wrapper: set state to busy before spawning
	...
	handle.subscribe((event) => {
		renderAgentOutput(event, deps.log);
		outputBuffer.push(event.kind === "raw" ? event.line : `[${event.kind}] ${(event as { message?: string; content?: string }).message ?? (event as { content?: string }).content ?? ""}`);
	});
	await handle.process.waitForOutput(/.*/, 300000);
	if (outputBuffer.length > 0) appendOutput(deps, iterDir, session.id, outputBuffer);
	updateSessionStatus(deps, iterDir, session.id, "completed");
	// Agent wrapper: set state back to idle after completion
