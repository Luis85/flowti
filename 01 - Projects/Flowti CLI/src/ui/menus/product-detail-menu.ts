/**
 * product-detail-menu.ts — Reduced detail menu for standalone products.
 *
...
export function buildProductDetailMenu(productPath: string, productName: string): MenuEntry[] {
	return [
		{
			key: "1",
			label: "Lifecycle",
			action: async () => {
				...
				return lifecycleStatusMenu(productPath, productName, "product");
			},
		},
		{
			key: "2",
			...
			action: async () => {
				...
				return requirementsMenu(productPath);
			},
		},
		{
			key: "3",
			...
			action: async () => {
				...
				return deliverablesMenu(productPath);
			},
		},
		{
			key: "4",
