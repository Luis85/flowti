...
describe("resolveE2EPaths", () => {
	...
	it("resolves all required path fields", () => {
		const e2e = resolveE2EPaths("/project", undefined, deps);
	...
	it("uses review config pluginId when provided", () => {
		const e2e = resolveE2EPaths("/project", { pluginId: "my-plugin" }, deps);
	...
	it("uses review config journeysDir when provided", () => {
		const e2e = resolveE2EPaths("/project", { journeysDir: "e2e/journeys" }, deps);
		expect(e2e.journeysDir).toBe("/project/e2e/journeys");
	});

	it("uses review config testVault when provided", () => {
		const e2e = resolveE2EPaths("/project", { testVault: "my-test-vault" }, deps);
	...
	it("env var takes precedence over review config", () => {
		...
		const e2e = resolveE2EPaths("/project", { testVault: "config-vault" }, deps);
