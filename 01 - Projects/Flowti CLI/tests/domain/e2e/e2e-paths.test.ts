...
const deps = { paths, proc } as const;
...
describe("resolveE2EPaths", () => {
	...
	it("resolves all required path fields", () => {
		const e2e = resolveE2EPaths("/project", undefined, "/vault", deps);
	...
	it("uses review config pluginId when provided", () => {
		const e2e = resolveE2EPaths("/project", { pluginId: "my-plugin" }, "/vault", deps);
	...
	it("uses review config journeysDir when provided", () => {
		const e2e = resolveE2EPaths("/project", { journeysDir: "e2e/journeys" }, "/vault", deps);
		expect(e2e.journeysDir).toBe("/project/e2e/journeys");
	});

	it("uses review config testVault when provided", () => {
		const e2e = resolveE2EPaths("/project", { testVault: "my-test-vault" }, "/vault", deps);
	...
	it("env var takes precedence over review config", () => {
		...
		const e2e = resolveE2EPaths("/project", { testVault: "config-vault" }, "/vault", deps);
