...
const mockPaths = {
	join: (...args: string[]) => path.posix.join(...args),
	resolve: (...args: string[]) => path.posix.resolve(...args),
	basename: (fp: string) => path.posix.basename(fp),
	dirname: (fp: string) => path.posix.dirname(fp),
	relative: (from: string, to: string) => path.posix.relative(from, to),
	isAbsolute: (fp: string) => path.posix.isAbsolute(fp),
	sep: "/",
} as never;
...
describe("validateConfigDeep", () => {
	...
	it("warns when reports.dir does not exist", () => {
		...
		const result = validateConfigDeep(config, "/project", fs, mockPaths);
