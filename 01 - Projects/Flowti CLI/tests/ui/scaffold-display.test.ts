...
} from "../../src/ui/scaffold-display.js";
...
describe("renderScaffoldResult", () => {
	it("renders created count and path", () => {
		renderScaffoldResult({ created: 3, outputPath: "/out", suggestions: [] });
	...
	it("skips suggestions section when empty", () => {
		renderScaffoldResult({ created: 1, outputPath: "/out", suggestions: [] });
...
describe("renderExportSaved", () => {
	it("renders total and output path", () => {
		renderExportSaved({ total: 5, outputPath: "/export.json" });
		const out = output();
...
describe("renderBundleImported", () => {
	it("renders imported count and vault", () => {
		renderBundleImported({ imported: 3, vault: "test-vault" });
		const out = output();
	...
	it("uses singular when imported is 1", () => {
		renderBundleImported({ imported: 1, vault: "v" });
