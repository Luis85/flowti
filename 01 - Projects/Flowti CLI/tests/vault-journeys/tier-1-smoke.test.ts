/**
 * tier-1-smoke.test.ts — Tier 1 smoke test runner for vault journeys.
 *
 * Loads all .journey files from tier-1-smoke/ and runs each step as a
 * Vitest test case. Uses the vault-test provider for setup/teardown.
 *
 * Run via: npx vitest run --config configs/vitest.vault.config.ts tests/vault-journeys/tier-1-smoke.test.ts
 */
