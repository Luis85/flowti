/**
 * tier-2-integration.test.ts — Tier 2 integration test runner for vault journeys.
 *
 * Loads all .journey files from tier-2-integration/ and runs each step as a
 * Vitest test case. Uses the vault-test provider for setup/teardown.
 *
 * Run via: npx vitest run --config configs/vitest.vault.config.ts tests/vault-journeys/tier-2-integration.test.ts
 */
