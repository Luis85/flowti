 ...
 * Opens when store.selectedAgent is non-null; closes via the x button.
 */
...
export class AgentPanel extends FlowtiElement {
	...
	private renderTabContent(agent: DashboardAgent) {
		...
		switch (tab) {
			...
			case "history":
				return html`<ft-game-panel-history .store="${this.store}" agentName="${agent.name}"></ft-game-panel-history>`;
			default:
				return nothing;
		}
	}
...
if (!customElements.get("ft-game-agent-panel")) customElements.define("ft-game-agent-panel", AgentPanel);
