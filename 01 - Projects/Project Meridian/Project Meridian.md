...
|**Tick Cycle**|Engine update loop with fixed timestep accumulator (§2.1)|Tick accumulator logic. One ExcaliburJS System (`MeridianTickSystem`) hosts the 2Hz accumulator. Game systems implement `GameSystem` interface and are dispatched by `TickRunner` — NOT registered as individual ExcaliburJS Systems (ADR deviation from original §30.2).|
