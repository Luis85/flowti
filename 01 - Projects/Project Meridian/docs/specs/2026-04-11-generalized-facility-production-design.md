...
**Status:** Draft (rev 6 — iteration 5 reviewer findings incorporated: perception pipeline (`perception-system.ts` / `perception.ts` / `component-data.ts`) added to migration plan, `SellAtMarket` line 19 added to `bt-actions-economy.ts` row, `WorldValidationLocation` interface `type` field drop documented)
**Date:** 2026-04-11
...
| `src/infrastructure/engine/debug-overlay.ts` | `loc.type` — 7 sites total: 232 (`LOCATION_ICONS[loc.type]`), 257-258 (icon + inline display), 336 (`locations.find(l => l.type === 'market')` — market header lookup), 637 (same market lookup in the trade panel), 820, 833 (display strings). `loc.production?.ticks_per_cycle` (239, 819); `loc.production !== null` (821, 971); `loc.production.<...>` (822, 972) — anomaly detector + progress display | Add a `FACILITY_TYPE_ICONS: Record<string, string>` map keyed by `facility_type.id`; 336 and 637 become `locations.find(l => l.facility_type === 'market_stall')`; progress reads `recipe.ticks_per_cycle`; anomaly detector reads `facility_type.primary_job` | Phase 2 (production refs) + Phase 3 (icon + market lookup refs) |
...
| `src/infrastructure/engine/world-loader.ts` | `loc.type` (206); `loc.production !== null` (207); `loc.production.job` (209); `loc.production.output.item_id` (210); `loc.production.input` (211) — builds the shape fed to `validateWorldConsistency`, NOT the BT perception projection | Projection includes `facility_type: string` and (for production facilities) resolved `active_recipe` fields. No more `type` field on this output. | Phase 2 |
| `src/domain/systems/behavior-agent.ts` | `PerceivedLocation` interface (15-20) declares `type: string` | Add `facility_type: string` field to `PerceivedLocation`. Keep `type: string` temporarily until Phase 3 (so Phase 2 doesn't break `bt-conditions-context.ts` / `bt-actions-needs.ts` / `bt-actions-economy.ts` mid-phase), then drop it in Phase 3 after all action/condition call sites migrate. | Phase 2 (add) + Phase 3 (drop `type`) |
| `src/infrastructure/entity/behavior-agent-factory.ts` | `resolveNearbyLocations()` (117-131) builds `PerceivedLocation` from `getLocations()` — currently copies `locData.type` into the projection | Copy `locData.facility_type` as well (new field). Both `type` and `facility_type` coexist during Phase 2; Phase 3 drops `type`. | Phase 2 (add) + Phase 3 (drop `type`) |
| `src/infrastructure/systems/perception-system.ts` | `locationInputs = locationList.map(l => ({ id: l.id, type: l.type, ... }))` (47) — copies `l.type` into the `PerceptionInput` shape | Add `facility_type` to the mapped entry in Phase 2; drop `type` in Phase 3. | Phase 2 (add) + Phase 3 (drop) |
| `src/domain/systems/perception.ts` | `PerceptionInput.locations: { id, type: string, pos }[]` (8); `nearbyLocations` pipeline copies `type` into state at line 31-36 | Add `facility_type: string` to the `PerceptionInput.locations` item shape and propagate through the pipeline. Drop `type` in Phase 3. | Phase 2 (add) + Phase 3 (drop) |
| `src/domain/core/component-data.ts` | `PerceptionState.nearbyLocations: { id, type: string, distance }[]` (69) — the cached perception snapshot the factory reads | Add `facility_type: string` to the cached tuple. This is what `behavior-agent-factory.ts:120-131` actually projects from (the factory looks the full `locData` back up via `getLocations()` as a fallback today, so functionally both work, but the cleanest path is to carry `facility_type` through the cache too). Drop `type` in Phase 3. | Phase 2 (add) + Phase 3 (drop) |
| `src/domain/systems/world-validation.ts` | `WorldValidationLocation.type: string` interface field (13) — read by world-loader when building the validation input shape; not used by `validateWorldConsistency` itself | Drop the `type` field from the `WorldValidationLocation` interface in the same Phase 2 commit that updates `world-loader.ts`. Otherwise world-loader's reduced shape would fail to assign to the interface. Add `facility_type` if any validation rule needs it (none do today). | Phase 2 |
| `src/infrastructure/entity/bt-actions-economy.ts` | `SellAtMarket` reads `locData?.type !== 'market'` (19) via `getLocations().find(...)`; `SeekMarket` filter `l.type === 'market'` (102) | Both retarget to `facility_type === 'market_stall'`. `SellAtMarket`'s check at line 19 becomes `locData?.facility_type !== 'market_stall'`. Same file gets the new `SeekWell()` action added alongside, filtering `l.facility_type === 'well' && l.stock.some(s => s.item_id === 'water' && s.quantity > 0)`. | Phase 3 |
| `src/infrastructure/entity/bt-actions-needs.ts` | `SeekFood` filter `l.type === 'food'` (183) in the perception-fallback branch. `SeekWater` (123, 137) and `SeekRest` (193, 205) are deleted in Phase 3, so their `l.type` reads disappear with them. `SeekFood` is **kept** and needs an actual replacement. | `l.facility_type === 'farm'` — food production is only from farms post-migration (Workshop makes tools, Smithy makes equipment). Market stalls still show up in the primary branch via `resolveNearbyFacilities().filter(f => f.stock.some(...))` which does not read `l.type`, so no second replacement is needed. | Phase 3 |
| `src/infrastructure/entity/bt-conditions-context.ts` | `AtLocation(type)` reads `locData?.type === type` (26); `NearLocation(type)` reads `l.type === type` (30). Both are parameterized conditions called from `base.mdsl` — currently only `[AtLocation, "market"]` at `base.mdsl:99`. | Change both conditions to read `facility_type` instead of `type`. Update the `base.mdsl:99` call site from `[AtLocation, "market"]` to `[AtLocation, "market_stall"]`. No other callers exist (grep-verified). The condition's string parameter now takes a facility type id. | Phase 3 |
