...
**Status:** Draft (rev 5 — iteration 4 reviewer findings incorporated: `SeekFood`/`AtLocation`/`NearLocation` call sites added to incidental sites table, `PerceivedLocation` interface + projection builder added to migration plan, two-phase `type` → `facility_type` coexistence window documented)
**Date:** 2026-04-11
...
| `src/infrastructure/entity/behavior-agent-factory.ts` | `resolveNearbyLocations()` (117-131) builds `PerceivedLocation` from `getLocations()` — currently copies `locData.type` into the projection | Copy `locData.facility_type` as well (new field). Both `type` and `facility_type` coexist during Phase 2; Phase 3 drops `type`. The PerceptionComponent's cached `nearbyLocations` stays unchanged shape-wise since it already stores only `{ id, distance }` tuples. | Phase 2 (add) + Phase 3 (drop `type`) |
| `src/infrastructure/entity/bt-actions-economy.ts` | `SeekMarket` filter `l.type === 'market'` (102) — existing action, kept but retargeted | Replace with `l.facility_type === 'market_stall'` reading the projected field from the extended `PerceivedLocation` shape. Same file gets the new `SeekWell()` action added alongside, filtering `l.facility_type === 'well' && l.stock.some(s => s.item_id === 'water' && s.quantity > 0)`. | Phase 3 |
