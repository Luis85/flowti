...
const NeedsConfigSchema = z.object({
	...
	activity_costs: z.record(z.string(), ActivityCostSchema).default({
		...
		seek_well:      { hunger: 1.3, thirst: 1.5, energy: 1.3 },
		seek_service:   { hunger: 1.1, thirst: 1.2, energy: 1.1 },
		...
		use_service:    { hunger: 0.3, thirst: 0.3, energy: 0 },
		...
		choose_service: { hunger: 1.0, thirst: 1.0, energy: 1.0 },
...
const EconomyConfigSchema = z.object({
	...
	overload_goods_threshold: z.number().default(5),
	equipment_repair_threshold: z.number().default(10),
...
export const GameConfigSchema = z.object({
	...
	// Baseline tick interval. 100ms = 10 ticks/sec. Lower values (down to min 10ms = 100 ticks/sec)
	// are produced when the user raises gameSpeed above 1x. At 10x (the top setting) the
	// effective interval is 10ms, which is aggressive but still feasible for a 3-agent world.
	tick_interval_ms: z.number().int().min(10).default(100),
	...
	items: z.record(z.string(), z.object({
		...
		equipment: { name: 'Equipment', baseValue: 12, maxCharges: 20 },
		tools: { name: 'Tools', baseValue: 8, maxCharges: 10 },
	}),
