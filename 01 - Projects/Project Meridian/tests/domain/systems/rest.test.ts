...
const defaultConfig = {
	owned_home: { recovery_rate: 4.0, mood_effect: 2 },
	public_shelter: { recovery_rate: 3.0, mood_effect: 0 },
	outdoors: { recovery_rate: 1.5, mood_effect: -3 },
};

describe('applyRest', () => {
	it('recovers energy at owned_home rate', () => {
		const result = applyRest({ currentEnergy: 50, restTier: 'owned_home' }, defaultConfig);
		expect(result.newEnergy).toBe(54);
	...
	it('recovers energy at public_shelter rate', () => {
		...
		expect(result.newEnergy).toBe(53);
