...
describe('serializeQuest', () => {
	it('produces a markdown block with all fields', () => {
		const entry: QuestLogEntry = {
			...
			timeline: [
				{ tick: 5, wallClock: 0, type: 'QuestGenerated', message: 'Generated' },
				{ tick: 25, wallClock: 0, type: 'QuestCompleted', message: 'Completed by Bram' },
			],
		};
		...
		expect(md).toContain('**Reward**: 15g');
		expect(md).toContain('**Duration**: 20t');
	...
	it('shows "(no events recorded)" for empty timeline', () => {
		...
		expect(serializeQuest(entry, resolveName)).toContain('(no events recorded)');
	});
});
