...
- [ ] GROUP BY on 1-3 dimension columns
- [ ] Aggregate functions: SUM, COUNT, AVG, MIN, MAX on locale-parsed numeric columns
...
- [ ] Time bucketing: month (`2026-02`), quarter (`2026-Q1`), year (`2026`)
- [ ] Column type hints: `number`, `date`, `string` with heuristic pre-fill from sample values
- [ ] 5 built-in locale presets: en-US, de-DE, en-GB, nl-NL, fr-FR + auto-detect
- [ ] Chain 3-way joins (A→B, result→C)
- [ ] Graceful handling of missing join keys and non-numeric values
...
- [ ] GROUP BY 1-3 dimensions with SUM, COUNT, AVG, MIN, MAX
- [ ] US number parsing: `"1,234.56"` → `1234.56`
- [ ] EU number parsing: `"1.234,56"` → `1234.56`
- [ ] FR number parsing: `"1 234,56"` → `1234.56`
- [ ] US date parsing: `"02/15/2026"` → month 2
- [ ] EU date parsing: `"15.02.2026"` → month 2
- [ ] ISO date parsing: `"2026-02-15"` → month 2
- [ ] Month, quarter, and year bucketing from parsed dates
- [ ] Column type hints control parsing behavior
- [ ] Non-numeric values in SUM skipped (treated as 0)
- [ ] Missing join key → excluded (inner) or filled (left)
