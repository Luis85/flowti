...
Quick Capture provides frictionless note creation via two ribbon icons ("Add Idea", "Add Feedback") and three command palette commands ("Quick Capture", "Add Idea", "Add Feedback"). Each action opens a minimal modal with a title input field. On submission, CaptureService creates a note in the configured capture folder with typed frontmatter (type, created timestamp, origin) and emits capture events. The user stays in their current context — no navigation required.

## Trigger

User clicks a ribbon icon ("Add Idea" lightbulb or "Add Feedback" message-circle) or invokes a command from the command palette ("Quick Capture", "Add Idea", or "Add Feedback").
...
- **System Response**: The ribbon click handler or command handler emits `ui.openQuickCapture` via EventBus. Ribbon icons emit with a pre-set type (`{ type: "idea" }` or `{ type: "feedback" }`). The "Quick Capture" command emits with no type (`{}`), which triggers the type selector in the modal. The "Add Idea" and "Add Feedback" commands emit with pre-set types, matching their ribbon counterparts.
...
- **User Action**: Types a note title in the text input field. Optionally selects a type from the dropdown (if visible).
- **System Response**: The modal captures the title value. Input field is full-width for comfortable typing. The modal heading reads "Quick Capture".
...
- **System Response**: The modal trims the title. If empty after trimming, nothing happens. Otherwise, the modal calls the `onSubmit` callback with a `CaptureInput` object (`{ title, type }`). The modal closes. main.ts forwards the input to `CaptureService.capture()`.
...
- **User Action**: (none — automatic)
- **System Response**: CaptureService reads the `captureFolder` setting (default: `00 - Connectivity/inbox`). It sanitizes the title (removes `\/:*?"<>|` characters) and builds the file path: `${captureFolder}/${sanitizedTitle}.md`. It generates frontmatter with the type in title case (e.g., "Idea", "Feedback", "Bug"), ISO timestamp, and `origin: quick-capture`. FileSystemClient creates the file with `createFolders: true`.
...
|----------|---------|---------|
| Capture type | idea, feedback, bug, or custom string | "idea" (ribbon) or user-selected (command palette) |
| Target folder | Any vault path | `00 - Connectivity/inbox` (from `captureFolder` setting) |
...
```yaml
---
type: Idea          # Title case: Idea, Feedback, Bug, etc.
created: 2026-02-21T14:30:00.000Z
origin: quick-capture
---
```

When a description is provided (future enhancement), it is included as:
```yaml
...
| Command: "Quick Capture" | user-selected | Command palette | Shown (idea/feedback/bug) |
...
- No Settings UI for `captureFolder` configuration yet (PBI-QC-001 I-1)
