...
describe("AnalyticsService", () => {
	...
	describe("onboarding checklist", () => {
		it("returns undefined before initialisation", () => {
			expect(service.getOnboardingChecklist()).toBeUndefined();
		});

		it("initialises checklist with installed milestone set", async () => {
			await service.initOnboardingChecklist();

			const checklist = service.getOnboardingChecklist();
		...
		it("updates milestones via partial merge", async () => {
			...
			await service.updateOnboardingChecklist({
		...
		it("update is no-op when checklist not initialised", async () => {
			const callCount = vi.mocked(storage.save).mock.calls.length;
		...
		it("dismisses the checklist", async () => {
			...
			await service.dismissOnboardingChecklist();

			expect(service.getOnboardingChecklist()!.dismissed).toBe(true);
		});

		it("dismiss is no-op when checklist not initialised", async () => {
			const callCount = vi.mocked(storage.save).mock.calls.length;
		...
		it("updates non-milestone fields", async () => {
			...
			await service.updateOnboardingChecklist({ collapsed: true });

			expect(service.getOnboardingChecklist()!.collapsed).toBe(true);
		});

		it("reset clears onboarding checklist", async () => {
			await service.initOnboardingChecklist();
			expect(service.getOnboardingChecklist()).toBeDefined();
