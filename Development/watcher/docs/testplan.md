...
| # | Use Case | User Story |
|---|----------|------------|
| UC-01 | Source-Only Sync (external → vault) | Importing external files into Obsidian |
| UC-02 | Vault-Only Sync (vault → external) | Exporting vault files to external tools |
| UC-03 | Bidirectional Sync | Editing files from both Obsidian and external editors |
| UC-04 | Subfolder Watching | Controlling recursive vs top-level watching |
| UC-05 | New Directory Detection | Auto-scanning new source subdirectories |

### Feature 2: Conflict Resolution

| # | Use Case | User Story |
|---|----------|------------|
| UC-06 | Conflict — Overwrite | Always taking the latest written version |
| UC-07 | Conflict — Skip | Never overwriting existing files |
| UC-08 | Conflict — Keep Newer | Keeping the most recently modified version |
| UC-09 | Conflict — Rename | Preserving both versions on conflict |
| UC-10 | Reverse Conflict Resolution | Separate conflict strategy for vault→source |

### Feature 3: Deletion & Move Handling

| # | Use Case | User Story |
|---|----------|------------|
| UC-11 | Deletion — Ignore | Preventing deletion propagation |
| UC-12 | Deletion — Trash | Syncing deletions safely via trash |
| UC-13 | Move Detection | Detecting renames as single operations |
| UC-14 | Orphan Cleanup | Cleaning up vault files without source counterpart |

### Feature 4: File Filtering

| # | Use Case | User Story |
|---|----------|------------|
| UC-15 | File Extension Filtering | Syncing only specific file types |
| UC-16 | Exclude Patterns | Excluding files/folders by glob pattern |
| UC-17 | Temp / System File Filtering | Ignoring temporary and OS files |
| UC-18 | Dotfile Filtering | Ignoring hidden files and directories |
| UC-19 | Symlink Protection | Skipping symbolic links safely |

### Feature 5: Reconciliation

| # | Use Case | User Story |
|---|----------|------------|
| UC-20 | Reconciliation on Start | Catching up after Obsidian was closed |
| UC-21 | Incremental Reconciliation | Skipping unchanged files for speed |
| UC-22 | Reconcile Worker Parallelism | Processing multiple files concurrently |
| UC-23 | Cancel Reconciliation | Stopping a running reconciliation |
| UC-24 | Concurrent Reconcile Guard | Preventing overlapping reconciliations |

### Feature 6: Reliability & Performance

| # | Use Case | User Story |
|---|----------|------------|
| UC-25 | File Stability Checks | Waiting for cloud-synced files to finish downloading |
| UC-26 | Retry on Transient Errors | Automatically retrying on temporary failures |
| UC-27 | Sync Loop Prevention | Preventing infinite sync ping-pong |
| UC-28 | Debounce Behavior | Consolidating rapid saves into single syncs |
| UC-29 | Backpressure / Queue Limits | Preventing unbounded memory consumption |

### Feature 7: Safety & Validation

| # | Use Case | User Story |
|---|----------|------------|
| UC-30 | File Size Limit | Preventing OOM on huge files |
| UC-31 | Path Traversal Protection | Preventing writes outside designated folders |
| UC-32 | Windows Path Length Validation | Handling MAX_PATH (260 char) limit |
| UC-33 | Unicode Path Normalization | Cross-platform Unicode path matching |
| UC-34 | Source Folder Validation | Graceful handling when source folder is missing |
| UC-35 | Overlapping Mapping Validation | Preventing two mappings writing to the same vault folder |

### Feature 8: Settings & Configuration

| # | Use Case | User Story |
|---|----------|------------|
| UC-36 | Mapping CRUD | Creating, editing, and deleting folder mappings |
| UC-37 | Polling Mode | Using polling for NAS / network drives |

### Feature 9: User Interface

| # | Use Case | User Story |
|---|----------|------------|
| UC-38 | Status Bar Display | At-a-glance sync status |
| UC-39 | Reconcile Progress Reporting | Real-time progress during reconciliation |
| UC-40 | Dashboard | Detailed watcher management and logs |
| UC-41 | Commands | Keyboard-accessible plugin actions |
| UC-42 | Watcher Health Monitoring | Identifying idle, warning, or error states |

### Feature 10: Persistence & Error Recovery

| # | Use Case | User Story |
|---|----------|------------|
| UC-43 | SyncState Persistence | Remembering sync state across sessions |
| UC-44 | SyncState Auto-Save | Debounced auto-save prevents data loss |
| UC-45 | Watcher Error Recovery | Handling chokidar errors gracefully |
| UC-46 | Watcher Close Timeout | Preventing hanging on unresponsive filesystems |
...
**Scenario 1.1: New file appears in source folder**
```gherkin
...
**Scenario 1.2: Existing file is modified in source folder**
```gherkin
Given a mapping with syncDirection "source-only"
...
**Scenario 1.3: File in subfolder is synced**
```gherkin
...
**Scenario 2.3: External changes are NOT pulled into vault**
```gherkin
Given a mapping with syncDirection "vault-only"
...
**Scenario 3.1: Source change syncs to vault**
```gherkin
...
**Scenario 4.1: Subfolders included when enabled**
```gherkin
...
**Scenario 4.2: Subfolders excluded when disabled**
```gherkin
...
**Scenario 5.1: New directory triggers incremental reconcile**
```gherkin
Given a mapping with watchSubfolders: true
...
**Scenario 5.2: Directory events are debounced**
```gherkin
...
**Scenario 5.3: Directory queue has backpressure limit**
```gherkin
...
**Scenario 6.2: Vault overwrites source file (reverse)**
```gherkin
...
**Scenario 11.1: Deleted source file remains in vault**
```gherkin
...
**Scenario 12.1: Deleted source file is trashed in vault**
```gherkin
...
**Scenario 13.2: Files with same size but different extension are NOT matched**
```gherkin
...
**Scenario 19.1: Symlinked file is skipped during sync**
```gherkin
...
**Scenario 19.2: Symlinked directory is skipped during walk**
```gherkin
...
**Scenario 20.4: Reconcile blocks watchers during execution**
```gherkin
...
**Scenario 21.3: Missing vault target triggers re-sync even if source unchanged**
```gherkin
Given incremental mode is enabled
...
**Scenario 21.4: Sync state is persisted after reconciliation**
```gherkin
Given incremental mode is enabled
...
**Scenario 22.1: Multiple files processed concurrently**
```gherkin
...
**Scenario 25.1: Unstable file is delayed until stable**
```gherkin
...
**Scenario 25.2: File that never stabilizes is skipped**
```gherkin
Given verifyFileStability is enabled
...
**Scenario 25.3: Stability checks disabled during reconciliation**
```gherkin
...
**Scenario 30.1: File over 100MB is skipped (forward sync)**
```gherkin
...
**Scenario 30.2: File over 100MB is skipped (reverse sync)**
```gherkin
...
**Scenario 30.3: File under 100MB is synced normally**
```gherkin
...
**Scenario 34.1: Missing source folder prevents watcher start**
```gherkin
...
**Scenario 34.2: Empty source folder prevents watcher start**
```gherkin
...
**Scenario 35.1: Identical target folders are rejected**
```gherkin
...
**Scenario 35.2: Nested target folders are rejected**
```gherkin
Given mapping A targets "vault/imported"
...
**Scenario 35.3: Non-overlapping target folders are accepted**
```gherkin
...
**Scenario 36.1: Create new mapping**
```gherkin
...
**Scenario 36.2: Edit existing mapping**
```gherkin
...
**Scenario 36.4: Validation rejects empty folders**
```gherkin
...
**Scenario 37.1: Polling detects file changes**
```gherkin
...
**Scenario 37.2: Polling interval is respected**
```gherkin
...
**Scenario 38.1: Normal mode shows stats**
```gherkin
...
**Scenario 38.2: Clicking status bar opens dashboard**
```gherkin
...
**Scenario 39.1: Progress shown during reconciliation**
```gherkin
...
**Scenario 39.2: Progress clears after reconciliation completes**
```gherkin
Given reconciliation has completed
Then the status bar should return to normal mode display
```

**Scenario 39.3: Per-mapping done notice**
```gherkin
...
**Scenario 40.1: Overview tab shows global stats**
```gherkin
...
**Scenario 40.2: Watchers tab shows per-mapping status**
```gherkin
...
**Scenario 40.3: Logs tab shows filtered log entries**
```gherkin
...
**Scenario 41.1: Restart watchers command**
```gherkin
...
**Scenario 41.2: Open dashboard command**
```gherkin
Given the user opens the command palette
...
**Scenario 43.1: State is saved on plugin unload**
```gherkin
...
**Scenario 43.3: Corrupted state file is handled gracefully**
```gherkin
...
**Scenario 44.2: Rapid changes are consolidated**
```gherkin
...
**Scenario 45.1: Chokidar error is logged and counted**
```gherkin
...
**Scenario 46.1: Slow close is timed out**
```gherkin
